...
ifeq ($(OS),Windows_NT)
	...
	/ := '\'
	WRITE_COVERAGERC_COMMAND := (Get-Host).Version#@echo "[run]`nomit = */invest/ui/*" | out-file .coveragerc -encoding utf8NoBOM
	ESCAPE_CHAR := `
...
TESTRUNNER := $(PYTHON) -m pytest -vs --cov=natcap.invest --cov-report='term' --cov-report='html' --cov-report='xml' --durations=0
