...
codesign_windows:
	$(GSUTIL) cp 'gs://stanford_cert/$(P12_FILE)' '$(BUILD_DIR)/$(P12_FILE)'
	$(SIGNTOOL) sign -f "$(BUILD_DIR)/$(P12_FILE)" -fd SHA256 -p "$(CERT_KEY_PASS)" -u 1.2.840.113635.100.4.9 "$(BIN_TO_SIGN)"
	$(SIGNTOOL) timestamp -td SHA256 -tr http://timestamp.sectigo.com $(BIN_TO_SIGN)
