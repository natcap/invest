...
class CLIHeadlessTests(unittest.TestCase):
    ...
    def test_run_no_report(self):
        """CLI: disable report generation with --no-report."""
        from natcap.invest import cli
        parameter_set_path = os.path.join(
            os.path.dirname(__file__), '..', 'data', 'invest-test-data',
            'coastal_blue_carbon', 'cbc_galveston_bay.invs.json')
        target = 'natcap.invest.coastal_blue_carbon.coastal_blue_carbon.MODEL_SPEC'

        with unittest.mock.patch(target) as mock_spec, unittest.mock.patch(
                'natcap.invest.cli.importlib.reload',
                side_effect=lambda module: module
        ):
            # pretend the model has a reporter, report should be generated by default
            mock_spec.reporter = 'foo'
            cli.main([
                'run',
                'coastal_blue_carbon',
                '--datastack', parameter_set_path,
            ...
            self.assertTrue(mock_spec.execute.call_args.kwargs['generate_report'])

            # report can be disabled with --no-report
            cli.main([
                'run',
                'coastal_blue_carbon',
                '--datastack', parameter_set_path,
            ...
            self.assertFalse(mock_spec.execute.call_args.kwargs['generate_report'])

            # report is not requested if none exists for the model
            mock_spec.reporter = ''
            cli.main([
                'run',
                'coastal_blue_carbon',
