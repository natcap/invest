...
from natcap.invest.spec import (
    IntegerOutput, NumberOutput, StringOutput, VectorOutput)
...
def _generate_mock_watershed_data(num_features, target_vector_path):
    """Generate vector containing mock watershed data for use in tests."""
...
def _get_mock_vector_spec():
    """Generate mock vector spec for use in tests.

    This spec is compatible with vector data generated by
    ``_generate_mock_watershed_data``."""
    return VectorOutput(
        id='watersheds',
        path='watersheds.gpkg',
        about='Watersheds vector.',
        geometry_types={'POLYGON'},
        fields=[
            IntegerOutput(
                id='ws_id',
                about='ID of the watershed.',
                units=u.none
            ),
            StringOutput(
                id='ws_name',
                about='Name of the watershed.'
            ),
            NumberOutput(
                id='calculated_value_1',
                about='Average water temperature.',
                units=u.degree_Celsius
            ),
            NumberOutput(
                id='calculated_value_2',
                about='Known corbomite deposits.',
                units=u.kilogram
            ),
        ]
    )
...
class TestGenerateResultsTableFromVector(unittest.TestCase):
    """Unit tests for ``vector_utils.generate_results_table_from_vector``."""
...
class TestGetVectorAttrTableCaption(unittest.TestCase):
    """Unit tests for ``vector_utils.get_vector_attr_table_caption``."""

    def test_caption_with_all_fields(self):
        spec = _get_mock_vector_spec()
        caption = vector_utils.get_vector_attr_table_caption(spec)
        expected_caption = [
            'ws_id:ID of the watershed. (Units: unitless)',
            'ws_name:Name of the watershed.',
            'calculated_value_1:Average water temperature. (Units: °C)',
            'calculated_value_2:Known corbomite deposits. (Units: kg)',
        ]
        self.assertEqual(caption, expected_caption)

    def test_caption_with_specified_fields(self):
        spec = _get_mock_vector_spec()
        fields_to_include = ['ws_id', 'calculated_value_2']
        caption = vector_utils.get_vector_attr_table_caption(
            spec, fields_to_include)
        expected_caption = [
            'ws_id:ID of the watershed. (Units: unitless)',
            'calculated_value_2:Known corbomite deposits. (Units: kg)',
        ]
