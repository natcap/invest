...
def validate(self, arg, valid_types=valid_types):
    ...
    for attr in attrs:
        ...
        if attr in {'name', 'about'}:
            ...
            if arg['type'] == 'csv':
                ...
                self.assertEqual(
                    set(arg[attr]['regexp'].keys()),
                    {'pattern', 'case_sensitive'})
                self.assertTrue(isinstance(
                    arg[attr]['regexp']['pattern'],
                    str))
                self.assertTrue(isinstance(
                    arg[attr]['regexp']['case_sensitive'],
                    bool))
    ...
    #         fn_name = f'test_args_spec_{model_name}'
    ...
    #                 errors.append(f'{model_name} arg {arg["name"]} errored: {ex}')
    #     print(errors)
...
def setup_tests():
    ...
    for model_name in model_names:
        ...
        def fn():
            importlib.import_module(f'natcap.invest.{model_name}')

        setattr(ValidateArgsSpecs, fn_name, classmethod(fn))
        print(dir(ValidateArgsSpecs))
