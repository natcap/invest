...
class SeasonalWaterYieldUnusualDataTests(unittest.TestCase):
    ...
    def test_lulc_values_not_subset_of_biophysical_table(self):
        """SWY test case where lulc value not found in biophyiscal table."""
        from natcap.invest.seasonal_water_yield import seasonal_water_yield
        ...
        # create custom LULC that has value not in biophysical table
        lulc_ras_path = os.path.join(args['workspace_dir'], 'lulc.tif')
        size = 100
        lulc_array = numpy.zeros((size, size), dtype=numpy.int8)
        lulc_array[size // 2:, :] = 1
        ...
        # Each pixel is 1x1 m
        pygeoprocessing.numpy_array_to_raster(
            lulc_array, -1, (1, -1), (1180000, 690000), project_wkt,
            lulc_ras_path)
        ...
        self.assertTrue(
            "The missing values found in the LULC but not the table are: {9}"
            in str(cm.exception))
    
    def test_soil_group_raster_values_invalid(self):
        """SWY test case where soil group raster value not in set {1-4}."""
        from natcap.invest.seasonal_water_yield import seasonal_water_yield
        ...
        # make custom soil group raster that has values other than 1-4
        soil_ras_path = os.path.join(args['workspace_dir'], 'soil_group.tif')
        size = 100
        soil_groups = 4
        soil_array = numpy.zeros((size, size), dtype=numpy.int32)
        ...
        # Each pixel is 1x1 m
        pygeoprocessing.numpy_array_to_raster(
            soil_array, -1, (1, -1), (1180000, 690000), project_wkt,
            soil_ras_path)
        ...
        self.assertTrue(
            "The other values found in the soil group raster were: {9}"
            in str(cm.exception))
