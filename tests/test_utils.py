...
class IsValidTests(unittest.TestCase):
    """Tests for natcap.invest.utils.is_valid."""
    def setUp(self):
        """Setup workspace."""
        self.workspace_dir = tempfile.mkdtemp()
    ...
    def test_simple_array(self):
        """utils: test on a small array"""
        from natcap.invest import utils

        # numbers very close to 0 should be recognized as nodata
        # relative tolerance defaults to 1e-5
        array = numpy.array([[0, 1], [0.001, 0.00001]])
        expected_nodata_0 = numpy.array([[False, True], [True, True]])
        expected_undefined_nodata = numpy.array([[True, True], [True, True]])

        self.assertTrue(numpy.array_equal(utils.is_valid(array, 0), expected_nodata_0))
        self.assertTrue(numpy.array_equal(utils.is_valid(array, None), expected_undefined_nodata))

    def test_pygeoprocessing_numeric_nodata(self):
        """utils: test numeric nodata value with raster_to_numpy_array"""
        from natcap.invest import utils

        # make a 10x10 array where each row has a different value
        # include a row of numeric nodata values (255)
        raster_data = [[255 for i in range(10)]]
        raster_data += [[i for j in range(10)] for i in range(9)]

        # make the expected output array 
        # with the nodata row labeled False (not valid)
        expected_result = [[False for i in range(10)]]
        expected_result += [[True for j in range(10)] for i in range(9)]

        srs = osr.SpatialReference()
        srs.ImportFromEPSG(26910)  # UTM Zone 10N
        project_wkt = srs.ExportToWkt()

        # write the raster to a file with the nodata value set to 255
        nodata_255_path = os.path.join(self.workspace_dir, 'nodata_255.tif')
        pygeoprocessing.numpy_array_to_raster(
            numpy.array(raster_data, dtype=numpy.int32),
            255, (1, -1), (1180000, 690000), project_wkt, nodata_255_path)

        self.assertTrue(numpy.array_equal(
            utils.is_valid(
                pygeoprocessing.raster_to_numpy_array(nodata_255_path),
                255),
            expected_result
        ))

    def test_pygeoprocessing_undefined_nodata(self):
        """utils: test undefined nodata value with raster_to_numpy_array"""
        from natcap.invest import utils

        # make a 10x10 array where each row has a different value
        # include a row of numeric nodata values (255) and a row of None
        raster_data = [[i for j in range(10)] for i in range(10)]

        # make the expected output array, all elements should be valid
        expected_result = [[True for j in range(10)] for i in range(10)]

        srs = osr.SpatialReference()
        srs.ImportFromEPSG(26910)  # UTM Zone 10N
        project_wkt = srs.ExportToWkt()

        # write the raster to a file with the nodata value undefined
        nodata_undefined_path = os.path.join(
            self.workspace_dir, 'nodata_undefined.tif')
        pygeoprocessing.numpy_array_to_raster(
            numpy.array(raster_data, dtype=numpy.int32),
            None, (1, -1), (1180000, 690000), project_wkt, nodata_undefined_path)

        self.assertTrue(numpy.array_equal(
            utils.is_valid(
                pygeoprocessing.raster_to_numpy_array(nodata_undefined_path),
                None),
            expected_result
        ))
