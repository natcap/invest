...
describe('Changelog', () => {
    ...
    test('Changelog modal contains only content relevant to the current version', async () => {
        ...
        let nonexistentVersionSectionFound = true;
        try {
            await findByRole('heading', { name: nonexistentVersion });
        } catch {
            nonexistentVersionSectionFound = false;
        }
        expect(nonexistentVersionSectionFound).toBe(false);
    });
});
