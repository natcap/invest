...
function SaveAsModal(props) {
  ...
  const [datastackType, setDatastackType] = React.useState('json'); // default to the JSON option
  const [relativePaths, setRelativePaths] = React.useState(false);

  async function browseSaveFile(event) {
    const {
      modelName,
      saveJsonFile,
      saveDatastack,
  ...
  function handleChange(event) {
    ...
    switch(name){
      case 'show':
        ...
        break;
      case 'datastackType':
        setDatastackType(value);
        break;
      case 'relativePaths':
        setRelativePaths(value);
        break;
      default:
        console.error(`handleChange() in SaveAsModal not covering ${name} ${value}`);
    }
  }
