...
def sort_to_disk(dataset_uri, dataset_index):
    ...
    for row_index in xrange(0, n_rows, row_strides):
        ...
        scores = -band.ReadAsArray(
            0, row_index, n_cols, row_strides).flatten()

        col_indexes = np.tile(np.arange(n_cols), (row_strides, 1))
        row_offsets = np.arange(row_index, row_index+row_strides) * n_cols
        ...
        left_index = np.searchsorted(sorted_scores, -nodata, side='left')
        right_index = np.searchsorted(sorted_scores, -nodata, side='right')
