...
def new_raster_from_base_uri(base_uri, *args, **kwargs):
    ...
    Args:
        base_uri (str): a URI to a GDAL dataset on disk.
...
def new_raster_from_base(base, output_uri, gdal_format, nodata, datatype,
                         fill_value=None):
...
def filter_fragments_uri(input_uri, value, patch_size, pixel_count, output_uri,
    ...
    LOGGER.info(
        "Filtering patches smaller than %i pixel(s) with value %i from %s.",
        patch_size,
        value,
...
def filter_fragments(src_array, value, patch_size, pixel_count, replace=None):
    ...
    small_fragment_mask = np.where(fragment_sizes < patch_size)
    small_fragment_sizes = fragment_sizes[small_fragment_mask]
    small_fragment_labels = fragment_labels[small_fragment_mask]
    LOGGER.debug("Found %i patches smaller than %i pixel(s).",
                 small_fragment_sizes.size - 1, patch_size)
    ...
    np.random.shuffle(small_fragment_labels)
    for label in small_fragment_labels:
        pixels_to_remove = np.where(label_im == label)
        if (pixel_count is not None) and (
                removed_pixels + pixels_to_remove[0].size > pixel_count):
            LOGGER.debug("Removing part of patch %i.", label)
            patch_mask = np.zeros_like(dst_array)
            ...
            pixels_to_remove = np.array(zip(
                pixels_to_remove[0], pixels_to_remove[1]))
        ...
        LOGGER.debug(
            "Removed patch %i of %i pixel(s).",
            label,
            pixels_to_remove[0].size)

        if (pixel_count is not None) and (removed_pixels == pixel_count):
...
if __name__ == "__main__":
    input_uri = "/home/mlacayo/workspace/data/ScenarioGenerator/input/"\
                "landcover.tif"
    patch_size = 5
    output_uri = "/home/mlacayo/workspace/data/ScenarioGenerator/"\
                 "despeckled.tif"
    value = 10
    replace = 1
    pixel_count = 2

    filter_fragments_uri(
        input_uri, value, patch_size, pixel_count, output_uri, replace)
